...
Overview of the WitChi workflows for detecting and reducing compositional bias in multiple sequence alignments. (A) The TEST workflow computes taxon-specific χ² scores against an empirical null from column permutation (100× default, configurable). Per-taxon empirical p-values flag problematic taxa but are diagnostic — sensitive to uneven branch lengths in the underlying tree — whereas the alignment-level empirical p-value is accurate and measures whether the alignment as a whole is biased. The PRUNE workflow iteratively removes the most biased columns — scored by Wasserstein-1 distance on Z-score quantiles (default), or Δχ²/Δχ⁴ — and a convergence check stops pruning when the alignment matches the null or no column's Δ exceeds the per-column (delta-null) expectation. (B) Example observed MSA and one corresponding column-permuted MSA, illustrating how taxon-specific biases are homogenised while preserving global composition; the bar plot compares taxon χ² scores between the observed and permuted alignments. (C) Left: per-column Δ scores, with the maximum Δ expected under a homogeneous alignment marked (horizontal line) and an example column flagged for removal (dashed box). Right: density distributions of taxon-specific χ² scores before and during the pruning loop, showing how pruning shifts scores toward the null until the alignment converges (right panel).
